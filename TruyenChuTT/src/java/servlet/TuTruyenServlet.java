 ...
 * @author USER
 */
@WebServlet(name = "TuTruyenServlet", urlPatterns = {"/tutruyen/*"})
public class TuTruyenServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (pathInfo) {
            case "/add":
                addToLibrary(request, response, user);
                break;
            case "/remove":
                removeFromLibrary(request, response, user);
                break;
            default:
                JsonObject jsonResponse = new JsonObject();
                jsonResponse.addProperty("success", false);
    ...
    private void showList(HttpServletRequest request, HttpServletResponse response, NguoiDung user)
        ...
        try {
            ...
            List<TuTruyen> danhSach = tuTruyenDAO.layDanhSachTruyenTrongTu(user.getId(), offset, ITEMS_PER_PAGE);
            
            // Đếm tổng số truyện
            int totalItems = tuTruyenDAO.demTruyenTrongTuTheoNguoiDung(user.getId());
            ...
            request.getRequestDispatcher("/User/TuTruyen.jsp").forward(request, response);
    ...
    private void checkStatus(HttpServletRequest request, HttpServletResponse response, NguoiDung user)
        ...
        try {
            ...
            boolean isSaved = tuTruyenDAO.kiemTraTruyenTrongTu(user.getId(), truyenId);
            
            jsonResponse.addProperty("success", true);
            jsonResponse.addProperty("isSaved", isSaved);
    ...
    private void addToLibrary(HttpServletRequest request, HttpServletResponse response, NguoiDung user)
            throws ServletException, IOException {
        ...
        try {
            ...
            // Tạo TuTruyen và thêm vào tủ
            TuTruyen tuTruyen = new TuTruyen();
            tuTruyen.setNguoiDung(user);
            tuTruyen.setTruyen(truyen);
            tuTruyen.setNgayLuu(LocalDateTime.now());
            
            boolean success = tuTruyenDAO.themTruyenVaoTu(tuTruyen);
            
            if (success) {
                jsonResponse.addProperty("success", true);
                jsonResponse.addProperty("message", "Đã thêm vào tủ truyện");
            } else {
                jsonResponse.addProperty("success", false);
                jsonResponse.addProperty("message", "Truyện đã có trong tủ rồi");
            }
    ...
    private void removeFromLibrary(HttpServletRequest request, HttpServletResponse response, NguoiDung user)
            throws ServletException, IOException {
        ...
        try {
            ...
            boolean success = tuTruyenDAO.xoaTruyenKhoiTu(user.getId(), truyenId);
            
            if (success) {
                jsonResponse.addProperty("success", true);
                jsonResponse.addProperty("message", "Đã xóa khỏi tủ truyện");
            } else {
                jsonResponse.addProperty("success", false);
                jsonResponse.addProperty("message", "Không tìm thấy truyện trong tủ");
            }
